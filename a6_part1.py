...
def train_model(X_train, y_train):
    ...
    print(f"\nEquation: Sales = {model.coef_[0]:.2f} × Test Scores + {model.intercept_:.2f}")
...
def make_prediction(model, hours):
    ...
    predicted_sales = model.predict(hours_array)[0]
    # TODO: Print the prediction with a clear message
    print(f"\n=== New Prediction ===")
    print(f"If hours is {hours}, predicted sales: {predicted_sales:.2f}%")
